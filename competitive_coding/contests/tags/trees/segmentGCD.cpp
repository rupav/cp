...
int get_gcd(int node, int tl, int tr, int l, int r){
    if(l > r) return 0;
    if(tl == l && tr == r) return t[node];
    int tm = (tl + tr)>>1;
    return combine(get_gcd(2*node, tl, tm, l, min(r, tm)),
                   get_gcd(2*node+1, tm +1, tr, max(l, tm+1), r));
}
