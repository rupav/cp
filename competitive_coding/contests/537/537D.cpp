...
int main()
{
    ...
    /// Compute factorial and inverse factorials modulo mod
    fac[0]=1;
    rep(i,1,n)fac[i]=(fac[i-1]*1ll*i)%mod;
    inv[n] = fmod(fac[n],mod-2);  /// calculate ((n)*(n-1)*(n-2)....(1))^-1 % mod using Little Fermat's theorem.
    ...
    {
        if(!buc[i])continue;
        /// Temporarily store dp using all characters in an array
        for(int k=0;k<=n;k++)
            temp_dp[k]= dp[k];
